...
const applyFilters = () => {
  ...
  router.get(route('dispatcher.orders'), params, { preserveState: true })
}
...
const setTab = (id) => {
  activeTab.value = id
  router.get(route('dispatcher.orders'), { 
    status: id,
    date_from: dateFrom.value,
    date_to: dateTo.value,
  }, { preserveState: true })
    ...
    <div v-if="pagination?.last_page > 1" class="mt-4 flex items-center justify-between">
      ...
      <div class="flex gap-2">
        <button 
          ...
          :key="page"
          @click="router.get(route('dispatcher.orders'), { page, status: activeTab, date_from: dateFrom, date_to: dateTo }, { preserveState: true })"
          :class="[
